...
model_opt = torch.optim.Adam([
    {'params': base_params, 'lr': 1e-5},   # base pré-treinada → conservador
    {'params': head_params, 'lr': 1e-5},   # nova camada → mais agressivo
...
if(filter_train): # treino para audios com filtro
    train_files_filtered, train_files = noise_reduction(train_files, args.freq, args.time, args.thresh, args.propdec)
    eval_files_filtered, eval_files = noise_reduction(eval_files, args.freq, args.time, args.thresh, args.propdec)
...
for epoch in range(50):
    model.train()
    train_acc, train_f1, _ = run_epoch(epoch,model, 
    ...
    model.eval()
    with torch.no_grad():
        val_acc, val_f1_score, true_val_f1_score = run_epoch(epoch, model, 
    ...
    #         epoch, model, LossCompute(model, None),
